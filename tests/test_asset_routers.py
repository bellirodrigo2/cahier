...
def test_getone_ok(id, path, mock_asset_service):
    ...
    mock_asset_service.get_one_by_webid.assert_called_with(
...
def test_getall_ok_w_query_params(mock_asset_service):
    ...
    assert response.status_code == 200
    expected_query_dict = {"fieldFilter": ["F1111", "F2222"], "selectedFields": "selll"}
    mock_asset_service.get_all_by_webid.assert_called_with(
        parent=parent_path,
        children=child_parent,
        webid=UUID(id),
        query_dict=expected_query_dict,
    )
    mock_asset_service.get_one_by_webid.assert_not_called()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        for oe in [{}, {"Name": "foo", "ClientId": "bar"}]
    ],
)
