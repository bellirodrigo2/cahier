...
class InconsistentTypeError(ObjError):
    ...
    def __init__(self, expected: ObjEnum, actual: ObjEnum):
        ObjError.__init__(
            self,
            f"Type Expectation Error. \
            Expected a type {expected=}, but got a(n) {actual=}",
        )


type_hierarchy_handler = create_exception_handler(
    status_code=400, initial_detail="ERROR"
)
inconsistent_type_handler = create_exception_handler(
    status_code=400, initial_detail="ERROR"
)
